    ...
    for i, nom_usuel_jotform in enumerate(file_jotform['Nom Usuel (animal)']):
        ...
        for j, nom_usuel_db in enumerate(file_db['Nom usuel']):
            ...
            if (nom_usuel_db == nom_usuel_jotform) and \
               (puce_db == puce_jotform):
                chien_trouve = True
                break

        if chien_trouve:
            message = f"Le chien ({nom_usuel_jotform}) existe dans la base de données."
            logger.info(message)
        else:
            message = f"Le chien ({nom_usuel_jotform}) n'existe pas dans la base de données."
...
def check_existence_and_similarity(field_name, jotform_value, db_data, log_messages, logger, category, i):
    ...
    if exists.any():
        # TODO hna lazem nchouf directement if not existe
        message = f"{field_name} ({jotform_value}) existe déjà dans la base de données."
        logger.info(message)
        log_messages.append(message)
    else:
        # Check if first name & familly name are not reversed
        ...
        if reverse_check.any():
            ...
            message_reverse = f"Le {field_name} ({jotform_value}) semblent avoir le nom/prénom inversés dans la base de données."
